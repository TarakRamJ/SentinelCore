    ...
    return (
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          <Box sx={{ width: 8, height: 8, borderRadius: "50%", backgroundColor: data.payload.color }} />
...
export const DashboardPage = () => {
  ...
  const handleQuickAction = async (actionType) => {
    ...
    try {
      ...
      setToast({
        open: true,
        message: err.response?.data?.message || "Something went wrong running that action.",
        severity: "error",
      });
    } finally {
  ...
  const resourceUsage = [
    ...
    { label: "Disk Usage", val: Math.round(data?.resourceSummary?.diskUsage ?? 67), icon: <DiskIcon fontSize="small" /> },
    { label: "Network Usage", val: Math.round(data?.resourceSummary?.networkUsage ?? 12), icon: <NetworkIcon fontSize="small" /> },
  ];
  ...
  const warningCount = Math.max(0, totalAssetsCount - healthyCount - criticalCount);
    ...
    <ThemeProvider theme={socTheme}>
      ...
      >
          ...
          {KPI_DEFS.map((kpi) => {
            ...
            return (
              ...
              >
                ...
                >
                  <Typography variant="subtitle2" color="text.secondary" noWrap sx={{ minWidth: 0, flex: 1 }}>
                    {kpi.label}
                  </Typography>
                  <Box
                    sx={{
                ...
                {loading ? (
                  <Skeleton variant="text" width="55%" height={28} sx={{ bgcolor: "rgba(255,255,255,0.06)" }} />
                ) : (
                  <Typography sx={{ fontWeight: 800, fontSize: "1.4rem", color: "#fff", lineHeight: 1 }}>
                    {displayVal}
                  </Typography>
                )}
              </Box>
              ...
              {resourceUsage.map((res, i) => (
                ...
                >
                  <Box sx={{ display: "flex", gap: 1, mb: 1, alignItems: "center" }}>
                    <Box sx={{ color: "text.secondary", display: "flex" }}>{res.icon}</Box>
                    <Typography variant="body2" color="text.secondary" sx={{ fontSize: "0.78rem" }}>
                      {res.label}
                    </Typography>
                  </Box>
                  <Typography sx={{ fontWeight: 700, fontSize: "1.1rem" }}>{res.val}%</Typography>
                  <LinearProgress
                    variant="determinate"
                    value={res.val}
                    sx={{
                      ...
                      "& .MuiLinearProgress-bar": {
                        borderRadius: 2,
                        backgroundColor: res.val > 80 ? "#EF4444" : res.val > 50 ? "#F59E0B" : "#10B981",
                      },
                    }}
                  />
                </Box>
        ...
        >
          ...
          >
            <CardContent sx={{ p: 2.5, flex: 1, display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
              ...
              <Box sx={{ display: "flex", justifyContent: "center", my: 1.5 }}>
                <Box sx={{ position: "relative", display: "inline-flex" }}>
                  ...
                  <CircularProgress
                    ...
                    thickness={5}
                    sx={{
                      color: securityScore > 70 ? "#10B981" : securityScore > 40 ? "#F59E0B" : "#EF4444",
              ...
              <Typography variant="body2" sx={{ color: "#9ca3af", fontSize: "0.75rem", textAlign: "center", px: 1 }}>
          ...
          <AssetHealthCard assetHealthData={assetHealthData} totalAssetsCount={totalAssetsCount} />
                  ...
                  <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", mb: 0.5 }}>
                    ...
                    <Chip
                      ...
                      sx={{
                        height: 18,
                        fontSize: "0.6rem",
                        backgroundColor: comp.status === "PASS" ? "rgba(16,185,129,0.16)" : "rgba(239,68,68,0.16)",
                        color: comp.status === "PASS" ? "#34d399" : "#f87171",
                      }}
                    />
                  </Box>
                  <Typography variant="caption" color="text.secondary">
                    Controls passed: {comp.passedControls}/{comp.totalControls} ({comp.scorePercentage}%)
                  </Typography>
                </Box>
              ))}
            </CardContent>
            ...
            <CardContent sx={{ p: 2 }}>
              ...
              <TableContainer>
                <Table size="small">
                  <TableHead>
                    <TableRow>
                      <TableCell sx={{ color: "text.secondary", p: "4px 6px", borderColor: "divider", fontSize: "0.65rem", textTransform: "uppercase" }}>
                        CVE ID
                      </TableCell>
                      <TableCell sx={{ color: "text.secondary", p: "4px 6px", borderColor: "divider", fontSize: "0.65rem", textTransform: "uppercase" }}>
                        CVSS
                    ...
                    {data?.topCriticalCVEs?.length > 0 ? (
                      data.topCriticalCVEs.map((cve) => (
                        <TableRow key={cve.id}>
                          ...
                          <TableCell sx={{ p: "6px", fontSize: "0.75rem", borderColor: "divider" }}>
                            {cve.cvssScore}
                          </TableCell>
                      ...
                      <TableRow>
                        <TableCell colSpan={4} align="center" sx={{ color: "text.secondary", py: 2, borderColor: "divider" }}>
        ...
        <Card sx={{ width: "100%", mb: 2 }}>
          <CardContent sx={{ p: 2 }}>
            <Typography variant="h6" sx={{ mb: 1.5, fontSize: "1.05rem", fontWeight: 700 }}>
            ...
            >
              {loadingLogs ? (
                [...Array(4)].map((_, idx) => (
                  <Skeleton key={idx} variant="rounded" height={68} sx={{ bgcolor: "rgba(255,255,255,0.03)", borderRadius: 1.5 }} />
                ))
              ) : auditLogs.length === 0 ? (
                ...
                auditLogs.slice(0, 4).map((log, idx) => (
                  ...
                  >
                    <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      ...
                      <Typography
                        variant="caption"
                        sx={{ fontSize: "0.68rem", color: "text.secondary", noWrap: true, maxWidth: "110px" }}
                        title={log.userEmail}
                      >
                        {log.userEmail}
                      </Typography>
                    ...
                    <Typography
                      variant="body2"
                      sx={{ fontSize: "0.8rem", mt: 0.5, fontWeight: 500, color: "#fff" }}
                      noWrap
                      title={log.action}
                    >
                      {log.action}
